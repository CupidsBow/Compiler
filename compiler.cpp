...
void expression(vector<IDwords>::iterator &it){
    ...
    while (it->word != ";"){// 遇到';'一条语句结束
        ...
        else oper_stack.push(it->word);// 优先级高
        it++;
    }
    // 弹出剩下的
    while(oper_stack.top() != "#"){
        ...
        if (op == "=") add_target_code(dst, op, dsc, " ", " ", " ");// 赋值运算
        else{
            vab = vab + 1;
            if(vab == 91) vab = '0';
            dsf = "tmp" + char_to_str(vab);
...
void create_asm(string file){
    ...
    if(!wfile.is_open()) cout << "无法创建汇编文件" << endl;
...
int main(int argc, char *argv[]){
    ...
    else cout << "正确使用格式：compile.exe [源文件] [汇编文件]" << "\n";
